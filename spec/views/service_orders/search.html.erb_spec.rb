...
describe 'service_orders/search.html.erb' do 
  context 'User search for service orders on application nav' do 
    ...
    it 'and the service order is finished in time' do 
      allow(SecureRandom).to receive(:alphanumeric).with(15).and_return('ABCDE12345678') 
      service_order = FactoryBot.create(
      ...
      FinishedServiceOrder.create!(service_order: service_order, delivery_date: 1.day.from_now)
      ...
      expect(page).to have_content 'Ordem de serviço <ABCDE12345678>'
      expect(page).to have_content "Data de início: #{ (Date.today).strftime("%d/%m/%Y") }"
      expect(page).to have_content "Data de encerramento: #{ 1.day.from_now.strftime("%d/%m/%Y") }"
      expect(page).not_to have_content "Motivo do atraso:"
    end

    it 'and the service order is finished too late' do 
      allow(SecureRandom).to receive(:alphanumeric).with(15).and_return('ABCDE12345678') 
      service_order = FactoryBot.create(
                                        :service_order, status: :finished,
                                        distance: 80, package_weight: 20, created_at: Date.today
      ...
      finished_so = FinishedServiceOrder.create!(service_order: service_order, delivery_date: 4.days.from_now)
      LatenessExplanation.create(justification: 'Dormi demais oh', finished_service_order_id: finished_so.id)
      ...
      expect(page).to have_content "Motivo do atraso: Dormi demais oh"
    end
  end
end